...
todoPage todos = renderHtml $ docTypeHtml $ do
  ...
  body $ do
    ...
    H.div ! class_ "todo-form" $ do
      H.form ! customAttribute "hx-post" "/htmx/todos"
             ! customAttribute "hx-target" "#todo-list"
             ! customAttribute "hx-swap" "outerHTML" $ do
        H.input ! type_ "text" ! name "title" ! placeholder "Add new todo..." ! required ""
...
todoItemHtml todo = H.div ! A.id (toValue $ "todo-" <> show (todoId todo))
                          ! class_ (if todoCompleted todo then "todo-item completed" else "todo-item") $ do
...
editTodoForm todo = renderHtml $ H.div ! A.id (toValue $ "todo-" <> show (todoId todo))
                                      ! class_ "todo-item" $ do
  H.form ! class_ "edit-form"
    ...
    H.button ! type_ "button" ! class_ "btn btn-delete"
